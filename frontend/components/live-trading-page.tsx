            ...
            {holdings.map((h, i) => {
              ...
              return (
                <div key={i} className={`rounded-lg border p-3 ${levelColor}`}>
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-zinc-500 mt-2">
                    ...
                    <span>T+5 <span className={Number(risk.t3_countdown) <= 1 ? "text-amber-400 font-medium" : "text-zinc-300"}>{Number(risk.t3_countdown)}天</span></span>
                    <span>距大哥黄 <span className="text-zinc-300">{Number(risk.yellow_distance).toFixed(1)}%</span></span>
                    <span>涨幅 <span className={profitPct >= 0 ? "text-green-400" : "text-red-400"}>{profitPct >= 0 ? "+" : ""}{profitPct.toFixed(2)}%</span></span>
                  </div>
