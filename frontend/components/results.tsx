  ...
  return (
    <div className="bg-zinc-900/50 rounded-lg border border-zinc-800 overflow-hidden shrink-0">
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
        ...
        <span className="text-xs text-zinc-500">共 {trades.length} 条</span>
      </div>
            ...
            {trades.map((t, i) => {
              ...
              return (
                <tr key={i} className="border-t border-zinc-800/50">
                  ...
                  <td className={`px-3 py-2 whitespace-nowrap font-mono ${isBuy ? "text-red-400" : "text-green-400"}`}>
                    {t.date}
                  </td>
                  <td className="px-3 py-2">
                    <span className={`text-xs px-2 py-0.5 rounded font-bold ${isBuy ? "bg-red-500/15 text-red-400" : "bg-green-500/15 text-green-400"}`}>
                      {isBuy ? "B" : "S"}
                    </span>
                  </td>
                  <td className={`px-3 py-2 text-right font-mono ${isBuy ? "text-red-400" : "text-green-400"}`}>
                    {t.price.toFixed(2)}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-zinc-400">
                    {t.volume}
                  </td>
                  <td className={`px-3 py-2 ${isBuy ? "text-red-300/90" : "text-green-300/90"}`}>
                    {t.reason || "—"}
                  </td>
                </tr>
              );
